...
```{r}
model_poisson_2 <- glm(rented_bike_count ~ is_weekday + hour + is_day + is_rush_hour + log1p(rainfall_mm) + log1p(snowfall_cm) + season
                       + temperature_c, data = bike_demand_filtered, family = poisson())
...
model_poisson_3 <- glm(rented_bike_count ~ is_weekday 
                       + hour + is_day + is_rush_hour
